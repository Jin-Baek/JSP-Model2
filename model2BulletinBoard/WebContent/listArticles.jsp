...
<%--HashMap으로 저장해서 넘오온 값들의 이름이 길어서 짧은 변수 이름으로 저장해서 사용한다 --%>
      ...
      <c:choose>
        ...
        <c:when test="${totArticles >100 }">  <!-- 글 개수가 100 초과인경우 -->
	      <c:forEach   var="page" begin="1" end="10" step="1" >
	         <!-- 섹션 값 1 초과부터는 앞 섹션으로 이동할 수 있는 pre 설정 -->
        ...
        <c:when test="${totArticles ==100 }" >  <!--등록된 글 개수가 100개인경우 : 첫 섹션의 10개 페이지만 표시하면 된다  -->
	      <c:forEach   var="page" begin="1" end="10" step="1" >
	        <a class="no-uline"  href="#">${page } </a>
	      </c:forEach>
        </c:when>
        
        <c:when test="${totArticles< 100 }" >   <!--등록된 글 개수가 100개 미만인 경우 : 표시되는 페이지의 수를 총 글의 갯수 / 10 + 1 까지 표시하면 된다. -->
